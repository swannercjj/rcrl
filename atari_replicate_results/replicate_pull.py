...
class Args:
    ...
    project_name: str = "Atari_Run_Replicate"
...
def extract_data(project_name, entity_name, data_dir):
    df = pd.DataFrame(columns=['env_id', 'seed', 'mean_episodic_return'])
    api = wandb.Api(timeout=20)
    runs = api.runs(f"{entity_name}/{project_name}")
    for run in runs:
        ...
        df.loc[len(df)] = [config.get('env_id'), config.get('seed'), dic.get('charts/episodic_return')]

        print(f"Data saved for run {run.id}")
        # os.remove(cache_path)

    file_path = os.path.join(data_dir, f"replicate_data.csv")
    df.to_csv(file_path)
